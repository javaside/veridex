...
export function ReleaseList({ kbId, releases, loading, error, onChanged, onRetry }: { kbId: string; releases: Release[]; loading: boolean; error: string | null; onChanged: () => void; onRetry: () => void }) {
  ...
  return (
    <section className="panel data-panel">
      ...
      {!loading && !error && releases.length === 0 && <div className="state-block compact"><h4>尚无索引发布</h4><p>文档处理完成后会自动创建并发布索引。</p></div>}
      {!loading && !error && releases.length > 0 && <div className="release-list">{releases.map((release) => {
        const pending = pendingId === release.releaseId
        return <article className="release-row" key={release.releaseId}><div className="release-version"><strong>v{release.versionNo}</strong><StatusBadge status={release.status} /></div><code>{release.indexName}</code><div className="row-actions">{release.status === 'PUBLISHED' && <><button className="text-button" disabled={pending} onClick={() => void act(release, 'rollback')}><ArrowsClockwise size={16} />回滚</button><button className="text-button" disabled={pending} onClick={() => void act(release, 'offline')}><CloudSlash size={16} />离线</button></>}<button className="text-button danger" aria-label={`删除发布 v${release.versionNo}`} disabled={pending} onClick={() => void act(release, 'delete')}><Trash size={16} />删除</button></div>{rowError[release.releaseId] && <p className="row-error" role="alert">{rowError[release.releaseId]}</p>}</article>
      })}</div>}
    </section>
  )
}
