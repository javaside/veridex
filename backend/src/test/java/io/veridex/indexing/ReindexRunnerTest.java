 ...
 * 退出动作，断言仍落在 getExitCode() 与 publish 调用序列上，语义不变。
 */
...
class ReindexRunnerTest {
    ...
    private ReindexRunner runner() {
        return new ReindexRunner(knowledgeBases, publisher) {
            @Override
    ...
    void republishesEveryKnowledgeBaseAndExitsZero() throws Exception {
        ...
        when(publisher.publish(any())).thenReturn(publishOutcome());
    ...
    void exitsNonZeroWhenAnyPublishFails() throws Exception {
        when(knowledgeBases.findAllIds()).thenReturn(List.of(UUID.randomUUID(), UUID.randomUUID()));
    ...
    private static ReleaseView publishResult() {
        return new ReleaseView(UUID.randomUUID(), 1, "ACTIVE", "veridex-x-1",
                "veridex-x-active", true, 0, 0);
    }

    private static PublishResult publishOutcome() {
        return new PublishResult(publishResult(), 0);
    }
}
