...
> 本文档基于当前实现整理，解释一次「上传文件 → 可被检索」的完整处理链路、Rabbit context 传播、固定失败观测和隐私边界。
...
`StructureFirstChunker`（`maxChars` / `overlap` 由「当前生效 Profile」的 chunking 维度驱动，默认 2000/80，未设置生效版本时回退 `ProfileDefaults`）：

1. **按 Markdown 标题切章节**：`^(#{1,3})\s+(.+)$`（1-3 级标题）命中即开新章节，标题同时作为该 chunk 的 `title` 和序号 `structurePath`（1、2、3…）
2. **无标题的纯文本**：整篇聚合为一个章节（标题用文件名）
3. **超长块硬切**：章节正文 > `maxChars` 字符时，按 `maxChars` 字符切段，段间重叠 `overlap` 字符（保证跨段语义不丢）

> 说明：分块参数在文档上传入库那一刻生效，属于快照参数。改配置后只对之后新入库的文档生效；存量文档需重新上传/重处理才会按新参数重切。
