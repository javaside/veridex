...
mkdir -p "${OFFLINE_DIR}/images" "${OFFLINE_DIR}/charts" "${OFFLINE_DIR}/scripts"
# 同步脚本副本到离线包（目标环境无仓库也要能执行 import/push）
for script in export-images.sh import-images.sh push-images.sh; do
  cp "${ROOT_DIR}/deploy/offline/scripts/${script}" "${OFFLINE_DIR}/scripts/${script}"
  chmod +x "${OFFLINE_DIR}/scripts/${script}"
done
